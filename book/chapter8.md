...
比如，你想把一个通用大模型训练成**更懂医学的模型**，可以把 Mid-training 理解为“让模型继续学习医学领域的教材和文献”。

> [!NOTE]
> 通用模型已经具备基本的语言能力，也掌握一些医学常识，但对罕见疾病、专业术语和诊疗文献的理解还不够深入。此时，可以收集医学教材、论文和专业文档，让模型在这些连续文本上继续进行**预测下一个 token 的训练**。这些数据不必整理成“用户提问—标准答案”的问答形式，直接使用原始医学文本即可。
> 例如，训练文本中出现：缺铁性贫血通常表现为血红蛋白降低、平均红细胞体积减小，血清铁蛋白……
> 模型需要根据前文预测后续内容。经过大量类似文本的训练，它会逐渐学习医学术语之间的关系，以及医学文献常见的表达方式。这就是一种 Mid-training；更具体地说，属于**领域自适应预训练（DAPT）**。
> 模型需要根据前文预测后续内容。经过大量类似文本的训练，它会逐渐学习医学术语之间的关系，以及医学文献常见的表达方式。这就是一种 Mid-training；更具体地说，属于**领域自适应预训练（DAPT）**。
...
>[!NOTE]
>  1. “沿用预训练的下一个 token 目标”：学习方式没变。
给模型一段医学教材，让它根据前文预测后面的 token；预测错了，就调整参数。它仍然通过阅读文本学习，并不一定需要“问题—标准回答”这样的数据。
> 2. “把数据分布收窄到目标领域”：读的材料更集中。
通用预训练时，模型读的是各种材料：新闻、小说、代码、百科等。医学 Mid-training 时，医学教材和论文的占比显著提高，让模型把更多训练机会用在医学知识和专业表达上。
但如果一直只读医学材料，模型原有的部分能力可能退化，这叫灾难性遗忘。因此，可以在医学数据中混入一些通用文本、代码等，让模型继续接触原来学过的内容。这些就是“通用保留数据”，作用类似于学专业课时也复习基础课，能缓解遗忘，但不能保证完全避免。
...
> [!NOTE]
> 损失函数：给模型的预测错误打分。模型把正确的下一个 token 预测得越不可能，损失就越大。
在某些 token 上计算损失：训练时，哪些位置的预测错误会被计入这个分数，用来调整模型参数。
> Mid-training：读连续材料。
法国位于欧洲，首都是巴黎。巴黎也是法国的政治、经济和文化中心……
训练时，通常会对文中大多数 token 的预测计算损失。模型既要学习“法国→巴黎”的关系，也要学习这段材料里的其他信息和表达。
> SFT：看问答示范。
> 用户：法国的首都是哪里？
助手：法国的首都是巴黎。
> 训练时，通常只对助手回答部分计算损失。用户的问题仍然会被模型读取，作为生成答案的条件，但一般不要求模型学习生成这个问题。
...
> [!NOTE]
> **怎样理解 LoRA：给已有模型加一个可训练的“小补丁”。** SFT、RL 说明“用什么信号教模型”：前者用标准回答示范，后者用奖励；LoRA 说明“训练时修改哪些参数”。因此，既可以用 LoRA 做 SFT，也可以用 LoRA 做 RL。
>
> **为什么全参数微调贵？** 模型参数是大量参与计算的数字，其中很多排列成矩阵。全参数微调允许调整所有参数，因此除了保存参数本身，还通常需要保存梯度（指示参数的调整方向）和优化器状态（帮助决定后续如何调整的历史信息）。几十亿参数都要保存这些额外记录，就会占用大量显存。
>
> **小补丁如何参与计算？** 对一个原始权重矩阵 $W$，LoRA 将它冻结，另外学习修正量，使实际计算使用的权重为 $W+\Delta W$。如果直接训练一个同样大的 $\Delta W$，仍然很贵，因此 LoRA 用两个小矩阵的乘积表示它：$\Delta W=BA$（这里省略缩放系数），只训练 $A$ 和 $B$。例如，原矩阵为 $4096\times4096$，约有 1678 万个参数；rank 为 8 时，两个小矩阵分别为 $4096\times8$ 和 $8\times4096$，合计只有 65,536 个可训练参数，约为原矩阵的 0.39%。相乘后仍然得到一个 $4096\times4096$ 的修正量，但不能任意独立调整其中的每个位置。
>
> **rank 是什么？** 可以把它理解为补丁允许使用的独立调整方向数量上限。rank 越小，可训练参数越少，调整空间受到的限制越多；rank 越大，调整更灵活，开销也更大。LoRA 能奏效，是因为已有模型适应某些任务时，可能只需要有限方向上的调整。例如，模型本来就会回答问题，现在让它更稳定地遵循某类问答示范，并不一定需要任意修改全部参数。但这不保证每个任务都能达到全参数微调的效果。
>
> **怎样看待上文的经验数字？** “参数量占 1%–5%”取决于 rank 和应用位置，并非固定范围；可训练参数减少到 1%，也不意味着总显存减少到 1%，因为原模型和训练中的中间结果仍占显存。“覆盖所有主要矩阵”“学习率约为全参微调的 10 倍”“RL 使用极小 rank”等结论有特定实验依据[^ch8-1]，应作为实验起点，而非适用于所有模型和任务的硬性规则。学习率指每次更新参数时的调整幅度，具体设置仍需验证。
>
> **冻结原权重不等于不会遗忘。** 挂上补丁后，模型的计算结果和行为仍会变化，原有任务的表现也可能下降。卸下补丁可以恢复基座模型，但不代表使用补丁时能保留全部原有能力。“多个 adapter 做多租户服务”则是共享同一个基座，为不同客户加载各自的小补丁，例如一个用于客服，另一个用于文本分类，避免为每个客户保存整套模型参数。
...
这段话的核心是：**如果模型偶尔能做对，强化学习就有成功样本可以鼓励；如果模型怎么尝试都做不对，只告诉它“错了”，它就很难知道该怎么改。**

> [!NOTE]
> 可以把模型想成一个正在练数学题的学生。
>
> **1. `pass@1` 和 `pass@k` 是什么？**
>
> 给同一道题，让模型独立尝试不同解法：
>
> - `pass@1`：只尝试一次，答对的概率。
> - `pass@k`：尝试 $k$ 次，其中**至少有一次答对**的概率。
>
> 假设每次答对的概率是 $p=10\%$，那么每次答错的概率就是 $90\%$。如果各次尝试近似独立，连续 10 次全部答错的概率是 $0.9^{10}$，因此：
>
> $$
> \operatorname{pass@}10=1-0.9^{10}\approx65\%
> $$
>
> 注意：这不代表模型一次回答的正确率变成了 65%，而是**给它 10 次机会，有约 65% 的概率能找到一个正确答案**。
>
> **2. 为什么“偶尔答对”对强化学习很重要？**
>
> 假设模型对一道题生成了 8 个解答，评分结果是：
>
> ```text
> 解答： A  B  C  D  E  F  G  H
> 奖励： 0  0  0  1  0  0  1  0
...
> 这里的 1 表示正确，0 表示错误。
>
> 训练就获得了一个可用信号：**D、G 这样的生成过程得到了更高奖励，以后应该提高它们出现的概率。**
>
> 这就是“正确策略已经处在模型分布里”的意思：模型已经有可能生成成功的解法，只是还不够稳定。“把概率质量放大”就是让这些成功解法更容易出现。
>
> **3. 如果 8 个解答全错，会怎样？**
>
> ```text
...
> ```
>
> 如果只看最终对错，这 8 个解答没有高低之分。即使其中一个只差最后一步，另一个完全胡说，评分也都是 0。
>
> GRPO 主要通过比较同一组解答的奖励来决定鼓励谁、抑制谁。**全是 0，就没有可供区分的奖励差异**，这就是“组内优势消失”。PPO 的具体计算不同，但同样缺少成功样本来指示有效的解题方向；不意味着它所有参数更新都会停止。
>
> **4. 为什么不能一直增加尝试次数？**
>
> 可以，但可能太贵。如果每次成功的概率只有百万分之一，那么平均需要约：
>
> $$
> 1/p=100\text{万次}
> $$
>
> 尝试才能遇到一次成功。这里的“平均”不是保证百万次内一定成功。
>
> 所以，“能力支持”指的是：**模型当前的能力，能不能让训练以可承受的成本采到成功解法。** 如果实测很多次都不成功，可以考虑先用成功示范做 SFT、从简单任务开始，或者提供中间步骤的反馈，再做强化学习。
>
> 原文也有一个需要注意的边界：**有限次数没采到成功，不等于证明模型绝对不会；偶尔答对，也不等于它已经掌握了可靠的方法，可能只是碰巧猜对。**
...
> [!NOTE]
> **RL 如何判断模型的结果是否正确？——评分器的设计**
>
> 通常由训练系统里的“评分器”来判断，而不是模型生成完就自动知道自己对不对。评分器可以是检查规则、执行测试的程序，也可以是奖励模型；具体采用哪种方式，取决于任务。
>
> 例如，对题目“$3x+2=11$，求 $x$”，训练程序可以提前保存标准答案 $x=3$。模型针对同一道题生成多个解答后，程序提取各自的最终答案：回答 $x=3$ 得到奖励 1，回答 $x=4$ 或没有给出答案则得到奖励 0。**标准答案不需要展示给正在解题的模型，评分程序知道即可。** 这些奖励随后用于更新模型参数，提高获得高奖励的生成过程的概率。
>
> 不同任务需要不同的评分依据：
>
> | 任务 | 如何评分 |
> | --- | --- |
> | 数学题 | 与标准答案比较，或检查数学表达式是否等价 |
> | 写代码 | 执行测试，检查程序输出是否符合要求 |
> | 游戏 | 由游戏环境给出胜负、得分 |
> | 操作网页 | 检查环境里是否真正完成目标，例如指定记录是否成功创建 |
> | 开放式回答 | 请人评价，或使用奖励模型、另一个模型按标准评分 |
>
> 数学、代码等任务通常有比较明确的验证依据，但检查也有覆盖范围：通过有限测试不等于程序在所有输入下都正确。开放式回答则更难有唯一标准，评分也更容易带有主观性或出现错误。
>
> 还要区分**结果正确与过程正确**。如果数学题只检查最终答案，模型即使推导错误、碰巧猜对，也可能拿到奖励。要判断中间步骤，就需要额外的过程检查。
>
> 因此，RL 直接优化的是**评分器给出的奖励**。设计评分器时，要让得分尽可能对应真实的任务完成情况，而不是只检查模型是否声称成功。评分器越能准确代表任务目标，训练越可能有效；如果它存在漏洞，模型也可能学会利用漏洞获得高分，这就是“奖励或环境有偏时，RL 同样可能对捷径过拟合”的含义。
...
> **怎样理解在线反馈提供的三个机会？**
>
> 这部分是在解释：**让模型亲自尝试、再根据反馈学习，能比只看固定示范多获得哪些训练机会。** 这里的“在线”指训练过程中持续产生新的交互数据，不是指连接互联网。
>
> 比如训练一个操作网页的模型：固定数据上的 SFT 给它看事先记录好的操作示范，例如“搜索商品 → 选择商品 → 加入购物车”；在线 RL 则让它实际操作网页，检查是否完成任务，再根据结果更新模型。
>
> **第一，可以尝试示范里没有教过的方法。** 假设人类示范一直是“打开菜单 → 找到搜索入口 → 输入商品名”，模型尝试时可能发现首页就有搜索框，直接输入更快。只要评分器能识别任务完成情况，以及节省步骤的价值，这种新方法就可能得到奖励、被训练强化。SFT 也可能依靠已有知识想到这种方法，但固定示范本身没有提供“这个新方法实际好不好”的反馈；在线交互提供了检验它的机会。当然，模型需要先尝试到这个方法，评分器也需要评得合理。
>
> **第二，有些任务“检查答案”比“写出答案”容易。** 比如要求模型写一个排序程序：准备 SFT 示范，需要先提供一份正确的程序；用 RL 训练，则可以让模型自己写，再运行测试，根据通过情况给奖励。你不一定要提前知道它应该怎么写，只需要能检查结果是否符合要求。这就是 RLVR（使用可验证奖励的强化学习）所利用的优势。但“RL 只需要可靠判断”是简化的说法：它仍然需要模型具备一定基础能力，并能通过探索找到有效答案；有限测试也可能漏掉错误。
>
> **第三，可以专门练习模型自己容易走进去的困境。** 假设示范里的操作一直很顺利：“首页 → 搜索结果 → 商品详情 → 加入购物车”。模型实际操作时却误点了一次，变成“首页 → 搜索结果 → 商家主页”。如果示范里从来没有“误入商家主页后如何返回”，模型就缺少相关示范，可能继续乱点，越走越偏。
>
> **前面的错误改变了后面会看到的页面，而这些页面又是模型不熟悉的，进一步增加了出错机会。** 这就是这里所说的协变量漂移：训练时看到的状态，与实际执行时遇到的状态不同。在线训练可以收集模型真正遇到的这些页面，让它学习恢复方法。不过，仅仅遇到困境还不够，还需要可用的奖励或教师指导。
>
> 文中的 $T$ 是一段任务中的操作步数。早期错误如果影响后续很多步，总损失可能比步数增长得更快。在特定理论条件下，固定示范训练的最坏情况界可含 $T^2$ 项；在线收集状态并获得合适监督，可以改善为随 $T$ 线性增长的界。**这不是“所有 SFT 都是平方误差、所有 RL 都是线性误差”的定律。**
>
> 最后提到的 **On-Policy Distillation（在轨蒸馏）**，就是把第三点和教师指导结合起来：学生自己操作，进入商家主页 → 教师针对这个实际状态给出指导 → 学生学习怎样返回并继续任务。它让学生在**自己实际会遇到的问题上接受指导**，而不是只反复学习教师一路顺利的操作记录。
